App_Core.AppTester = (function(mainTestModule) {
    ...
    unitTests[unitTests.length] = CookieTester.createCookie = function(
        ...
        App_Core.CookieHandler.createCookie(name, value);
        var cookieVal = App_Core.CookieHandler.getCookieValueAsString(
            name);
        if (cookieVal === value) {
            App_Core.log(1, 'createCookie has passed',
                'green');
            totalPasses++;
        } else {
            App_Core.log(3, 'createCookie has Failed!');
            totalErrors++;
        }
    };
    ...
    unitTests[unitTests.length] = CookieTester.populateCookie =
        function(name, value) {
            ...
            var cookieCurrentVal = App_Core.CookieHandler.getCookieValueAsString(
                name);
            //adds the value with delimiter
            App_Core.CookieHandler.populateCookie(name, value);
            var cookieVal = App_Core.CookieHandler.getCookieValueAsString(
                name);
            var cookieDelimiter = App_Core.CookieHandler.getCookieDelimiter();
            value = cookieCurrentVal + cookieDelimiter + value;
            if (cookieVal === value) {
                App_Core.log(1, 'populateCookie has passed',
                    'green');
                totalPasses++;
            } else {
                App_Core.log(3, 'populateCookie has Failed!');
                totalErrors++;
            }
        };
    ...
    unitTests[unitTests.length] = CookieTester.findValueInCookie =
        function(name, value, shouldFail) {
            ...
            //gets value without the delimiter
            var valueIsFound = App_Core.CookieHandler.findValueInCookie(
                name, value);
            if (valueIsFound) {
                App_Core.log(1, 'findValueInCookie has passed',
                    'green');
                totalPasses++;
            } else {
                ...
                App_Core.log(3, 'findValueInCookie has Failed!');
                totalErrors++;
            }
        };
    ...
    unitTests[unitTests.length] = CookieTester.addValueToCookie =
        function(name, value, shouldModify) {
            ...
            var cookieCurrentVal = App_Core.CookieHandler.getCookieValueAsString(
                name);
            //adds the value with delimiter
            App_Core.CookieHandler.addValueToCookie(name, value);
            var cookieModifiedVal = App_Core.CookieHandler.getCookieValueAsString(
                name);
            var cookieDelimiter = App_Core.CookieHandler.getCookieDelimiter();
            ...
            if (cookieModifiedVal === value) {
                App_Core.log(1, 'addValueToCookie has passed',
                    'green');
                totalPasses++;
            } else {
                App_Core.log(3, 'addValueToCookie has Failed!');
                totalErrors++;
            }
        };
    //append a new value to the cookie
    ...
    unitTests[unitTests.length] = CookieTester.getCookieValueAsArray =
        function(name, value1, value2) {
            ...
            // create the cookie
            App_Core.CookieHandler.createCookie(name, value1);
            // add the second value to cookie
            App_Core.CookieHandler.addValueToCookie(name, value2);
            // get cookie's value
            var cookieVal = App_Core.CookieHandler.getCookieValueAsArray(
                name);
            if (App_Core.checkIfArray(cookieVal)) {
    ...
    unitTests[unitTests.length] = CookieTester.removeValueByValue =
        function(name, value1, value2) {
            ...
            App_Core.CookieHandler.removeValueByValue(name,
                value1);
            // get cookie's value
            var cookieVal = App_Core.CookieHandler.getCookieValueAsString(
                name);
            if (cookieVal === value2) {
                App_Core.log(1, 'removeValueByValue has passed',
                    'green');
                totalPasses++;
            } else {
                App_Core.log(3, 'removeValueByValue has Failed!');
                totalErrors++;
            }
        };
    ...
    unitTests[unitTests.length] = CookieTester.deleteCookie = function(
        ...
        App_Core.CookieHandler.createCookie(name, value);
        // delete the cookie
        App_Core.CookieHandler.deleteCookie(name);
        // get cookie's value
        var cookieVal = App_Core.CookieHandler.getCookieValueAsString(
            name);
        if (!cookieVal) {
            App_Core.log(1, 'deleteCookie has passed',
                'green');
            totalPasses++;
        } else {
            App_Core.log(3, 'deleteCookie has Failed!');
            totalErrors++;
        }
    };
    CookieTester.cleanup = function() {
        App_Core.CookieHandler.deleteAllCookies();
        totalErrors = 0;
        totalPasses = 0;
    };
    // run all unit tests
