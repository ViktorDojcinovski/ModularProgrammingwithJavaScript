...
define(['MainCore', 'Utilities'], function(App_Core) {
    ...
    function saveArrayToLocalStorage(key, value, encode) {
            ...
            if (checkLocalStorageForkey(key) === false) {
                if (App_Core.checkIfArray(value)) { //** about using core ifArray function
                    if (encode) {
                        value = encodeURI(value);
                    }
    ...
    function appendObjectToLocalStorageArray(key, objToSave, encode) {
            ...
            if (storedValue) {
                if (App_Core.checkIfArray(storedValue)) {
    ...
    function initialize() {
        if (App_Core.testLocalStorage()) {
            App_Core.log(1, 'LocalStorage is availble.',
                "green");
        } else {
            App_Core.log(3, 'LocalStorage is not availble!');
        }
        App_Core.log(1,
            'StorageHandler Module has been initialized...',
            "blue");
    }
    return App_Core.StorageHandler = {
